...
## 2026-03-05 — 轨迹外延 bug 修复

- 修复 `_fallback_replan` 中走廊-控制点映射错误：每个碰撞点只移向对应段的走廊中心
- 修复移动方向公式：控制点移向中心而非越过中心
- 新增路径去重 (`_deduplicate_path`) 和贪心简化 (`_simplify_path`) 后处理
- 修复 attempt>=2 插入点的索引偏移问题
- `main.py` 碰撞修复后增加去重+简化步骤
- 测试：20/20 零碰撞，路径长度比中位 1.10 (最大 1.58)
